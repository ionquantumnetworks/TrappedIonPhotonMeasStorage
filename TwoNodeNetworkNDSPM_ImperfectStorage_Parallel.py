...
def simloop(timestr,i):
    ...
    #Storage timers, useful for non-infinite storage
    TimerA = 0;
    TimerB = 0;
    tauStorage = 100; #1/e storage time, us
    ...
    #track the photon's arrival time to the BSA
        ...
        #----------ADDING IN TIMER FOR STORAGE------------#
        if StoredA == True:
            TimerA+=1;
        if StoredB == True:
            TimerB+=1
    
        #-----------Photon Transmission Timers------------#
        if TranA > 0:
            if TranA == 1:
        ...
        #-----------Checking for two-photon interference-----------#
        ...
        if StoredA == True and StoredB == True:
            #------Here is where we add calculation if photon made it through fiber, storage, and detector-------#
            #-----------------MOVE THIS DOWN TO WHEN StoredA AND StoredB ARE TRUE---------------------------------------------
            photonTransmittedA = random.randint(1,10000);
            photonTransmittedB = random.randint(1,10000);
            StorageProbA = StorageEff*math.exp(-TimerA*Period/tauStorage)
            StorageProbB = StorageEff*math.exp(-TimerB*Period/tauStorage)
            if photonTransmittedA <= photonFiberProb*StorageProbA:
                PhotonA = 1;
                #print("yes")
            if photonTransmittedB <= photonFiberProb*StorageProbB:
                PhotonB = 1;
                #print("yes")
...
if __name__ == "__main__":
    ...
    plotlist=[];
    with Pool(3) as p:
        plotlist.append(p.map(func, range(0,510,10)))
    
    with open("Imperfect_Storage_100us_"+timestr+"final.csv", 'a+') as f:   
        writer = csv.writer(f)
